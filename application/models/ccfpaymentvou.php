...
class Ccfpaymentvou extends CI_Model {
    ...
    public function updateledgerposting1() {
        ...
        $this->db->where('companyId', $this->sessiondata['companyid']);
        $this->db->where('voucherNumber', $paymentMasterId);
        $this->db->where('ledgerId', $ledgerId);
    ...
    public function updatepartybalance() {
        ...
        $this->db->where('companyId', $this->sessiondata['companyid']);
        $this->db->where('againstvoucherNo', $paymentMasterId);
        $query=$this->db->update('partybalance', $data);
        return $query;
    }
    ...
    public function currentbalance($paidid) {
        $companyId = $this->sessiondata['companyid'];
        $query = $this->db->query("SELECT sum(debit) as debit ,sum(credit) as credit FROM `partybalance` where companyId='$companyId' AND (ledgerId='$paidid' AND voucherType='Purchase Invoice') or (ledgerId='$paidid' AND voucherType='PaymentVoucher')");
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $debit =  $row->debit;
            $credit = $row->credit;
